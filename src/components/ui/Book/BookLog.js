...
class BookLog extends PureComponent<Props> {
  ...
  render() {
    ...
    const totalWords =
      Number(
        bookList.reduce((acc: number, book) => acc + Number(book.pageCount), 0)
      ) * wordsPage;
    const totalMin = Math.floor(totalWords / result.speedReadingWords);

    return (
      <div
        className={`log-list-book-wrapper book-block-item${
        ...
        {result.speedReadingWords > 0 ? (
          <div className="app-main-text-result-book">
            <h3>
              ...
              <span className="special">по 60 минут в день.</span>
              <br />
              Вы прочитаете...
            </h3>
          ...
          <p>
            Для того чтобы посчитать время необходимое для прочтения вашего
            журнала сначала пройдите тест на скорость чтения.
          </p>
        )}
      </div>
    );
